...
const Slider: FC<SliderProps> = ({
  ...
  const handleOnInput = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setProgress(String((Number(value) * 100) / Number(max ?? "100")))
  ...
  return (
    <label className="flex flex-col gap-1">
      ...
      <input
        ...
        className="relative top-[-16px] h-3 w-60 cursor-pointer appearance-none rounded-full bg-transparent p-0 slider"
        type="range"
      />
    </label>
  )
  ...
  return (
    <div className="flex h-3 w-60 overflow-hidden rounded-full bg-slate-200 shadow">
      <div className="rounded-full bg-slate-300 text-end">
        <div className="size-3 rounded-full bg-white shadow"></div>
      </div>
